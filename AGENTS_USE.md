...
| Single LLM provider (Anthropic), no OpenRouter multi-provider | The ReAct orchestration loop uses Anthropic's native `tool_use`/`tool_result` message format — not a generic standard. Routing the orchestrator through OpenRouter would require a per-provider translation layer for tool schemas and parallel tool call handling. That complexity is not justified: the cost optimization goal is already met by using `claude-haiku-4-5-20251001` for extraction-only tasks (~80% cost reduction vs Sonnet, no quality loss). OpenRouter is retained in config for non-tool-use calls if a future operator needs it. | Provider-agnostic abstraction layer translating Anthropic tool schemas to OpenAI function calling or Gemini tool format |
